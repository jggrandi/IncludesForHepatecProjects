...
#include <ImprovedSurfaceNets.h>

using namespace std;

/**
*/
CImprovedSurfaceNets::CImprovedSurfaceNets(void)
{
...
(0,0,0)
*/
bool CImprovedSurfaceNets::GenerateMesh(float isoLevel)
{
	int width,
		height, 
		depth,
...
/**
*/
bool CImprovedSurfaceNets::ApplyLaplacianSmoothing(int numberOfIterations)
{
...
/**
*/
void CImprovedSurfaceNets::SetupVertexPool(void)
{
...
/**
*/
void CImprovedSurfaceNets::ClampToBoundingBox(const float *oldVertexCoord, float *newVertexCoord, const float boundingBox[2][3])
{
	int i;
	float scale;
	bool clamp;
...
/**
*/
bool CImprovedSurfaceNets::LoadVertexNeighborhoodInfo(void)
{
	int triangleCount,
		i;
...
/**
*/
bool CImprovedSurfaceNets::ProjectMeshOnIsosurface(void)
{
	int vertexCount,
		x,
		y,
	...
	const float *vertex,
		*normalAnswer;
	// Sampling cell diagonal length, which will be used to calculate the 
	// vertex stepsize.
	float cellDiagonalLength,
		// Vertex stepsize limits.
		stepsizeMin,
		stepsizeMax,
		newVertexCoord[3],
		value,
		vertexDistanceToIsosurface,
		// Says when we are in (positive) or out (negative) the isosurface.
		functionSignalOnVertex,
		boundingBoxMargin,
		boundingBox[2][3];
	...
	// Calculates the length of cell diagonal.
	cellDiagonalLength = MyMath::Magnitude(m_cellWidth, m_cellHeight, m_cellDepth);

	// Calculates the minimum and maximum stepsize.
	stepsizeMin = cellDiagonalLength/100.0f;
	stepsizeMax = cellDiagonalLength/10.0f;
	...
	{
		vertex = m_triangleMesh->GetVertex(i);
		...
		for (j=0; j<3; j++)
			newVertexCoord[j] = vertex[j];

		// How many iterations are enough?
		for (j=0; j<10; j++)
		{
			value = m_image->GetValue(newVertexCoord[0], newVertexCoord[1], newVertexCoord[2]);

			vertexDistanceToIsosurface = fabs(value - m_isoLevel);

			if (vertexDistanceToIsosurface < stepsizeMin)
				break;
			else if (vertexDistanceToIsosurface > stepsizeMax)
				vertexDistanceToIsosurface = stepsizeMax;

			// Reduces the vertex speed when it get close to the isosurface.
			vertexDistanceToIsosurface *= vertexDistanceToIsosurface;

			functionSignalOnVertex = 1.0f;

			if (value > m_isoLevel)
				functionSignalOnVertex = -1.0f;

			normalAnswer = m_image->GetNormal(newVertexCoord[0], newVertexCoord[1], newVertexCoord[2]);

			for (k=0; k<3; k++)
				newVertexCoord[k] += normalAnswer[k]*vertexDistanceToIsosurface*functionSignalOnVertex;
		}

		ClampToBoundingBox(vertex, newVertexCoord, boundingBox);

		m_triangleMesh->SetVertex(i, newVertexCoord[0], newVertexCoord[1], newVertexCoord[2]);
	}
...
/**
*/
int CImprovedSurfaceNets::GenerateVertex(float x, float y, float z)
{
	int vertexIndex;
...
/**
*/
int CImprovedSurfaceNets::GetVertexId(int x, int y, int z) const
{
...
/**
*/
void CImprovedSurfaceNets::RenameElements(void)
{
